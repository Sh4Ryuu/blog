...
export default function BlogPassword({ onAuthenticated, blogTitle }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/api/blogs/check-password`, {
        method: "POST",
        headers: {
      ...
      >
        <div className="text-center mb-4">
          ...
          <p className="text-sm mb-4" style={{ opacity: 0.8 }}>
            This blog post is protected. Please enter the password to view the
            content.
          </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {error && (
            <div className="text-red-500 text-sm font-mono mt-2">{error}</div>
          )}
